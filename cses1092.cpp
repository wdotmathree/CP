...
int main() {
	...
	if ((int)((n + 1) * (n / 2.)) % 2) {
		...
		// We double that number (the double cancels with the half from Gauss' formula) and compare it with the sum of the array.
		// If they are equal, that's our solution, and if not, there is no solution
		if ((lb + rb) * (rb - lb + 1) != (long long)((n + 1) * (n / 2.))) {
			cout << "NO\n";
		} else {
			cout << "YES\n" << ceil(n / 2.) << '\n';
			for (int i = lb; i < rb; i++) {
